...
namespace DevExpress.Web.Demos {
    ...
    public static class FullScreenModeUtils {
        ...
        const string ImageUrlQrCodeFormat = "http://chart.apis.google.com/chart?cht=qr&amp;chs=82x82&amp;chl={0}&amp;chld=H%7c0";
        public static string GetImageUrlQrCode(string resolvedUrl) {
            HttpRequest request = HttpContext.Current.Request;
            string url = string.Format("{0}://{1}{2}", request.Url.Scheme, request.Url.Authority, resolvedUrl);
            return string.Format(ImageUrlQrCodeFormat, HttpUtility.UrlEncode(url));
        }
    }
    ...
    public static class Utils {
        ...
        public static void RegisterCurrentMvcDemoOnCallback() {
            ...
            int controllerNamePartIndex = demoUriParts.Length > 2 ? demoUriParts.Length - 2 : -1;
            if(controllerNamePartIndex > -1) {
                string controllerNamePart = demoUriParts[controllerNamePartIndex];
                if(Request.AppRelativeCurrentExecutionFilePath.Contains(controllerNamePart)) {
                    controllerName = controllerNamePart.Replace("/", "");
        ...
        public static void InjectIECompatModeMeta(Control parent, int compatibilityVersion) {
            if(!RenderUtils.Browser.IsIE || RenderUtils.Browser.Version >= 10 || RenderUtils.Browser.Version < compatibilityVersion + 1)
                return;
            ASPxWebControl.SetIECompatibilityMode(compatibilityVersion);
        }
